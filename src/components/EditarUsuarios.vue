...
methods: {
  async buscarDadosUsuario()
  {
      ...
      try {
          const response = await axios.get(`https://localhost:7269/api/v1/usuarios/buscar/${usuarioId}`);
  ...
  async atualizarUsuario() {
    try {
      const response = await axios.put(`https://localhost:7269/api/v1/usuarios/atualizar/${this.usuario.id}`, this.usuario);
      ...
      Swal.fire({
        title: "Erro!",
        text: "Falha ao ataulizar o usuário.",
        icon: "error",
        confirmButtonText: "OK"
      });
      console.error(error);
